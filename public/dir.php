<?php 
	...
	*/
	...
	class dir{
		...
		function dirurl($dir){
            $urf=str_replace($this->sdir,'',$dir);
			return "?dir=".rawurlencode($urf);
		}
		function value($value){
            $urf=strstr($value,'/file/');
			return $urf;
		}
		function type($file){
			$ex=$this->ex($file);
		...
		function pre(){
            $mm=str_replace($this->sdir,'',$this->dir);
			$dir_array=explode("/", $mm);
			$num=count($dir_array);
			if($num>=2){
...
<?php echo $_SERVER['HTTP_REFERER'] ?>
				"
				><h2 class="btn btn-primary mgitb"><span class="glyphicon glyphicon-chevron-left " id="back"></span></h2></a>
			</div>
			<div class="col-md-9">
				<h4 class='mgitb'>
<?php		
...
?>				</h4>
			...
			<div class='col-md-2'>
				<a href='/user/magnet/mylist.html' class='btn btn-info mgitb'>我的下载</a>
			</div>
		</div>
